...
def parse_kstyle(soup, group_name: str):
    ...
    articles : List[dict]
    """
    base_url = "https://news.kstyle.com"
    articles_soup = soup.find_all('li', class_='mdMN02Item')

    def inner(article) -> dict:
        article_title = article.find(
            'h3', class_='mdMN02Ttl').text.replace("\n", "")
        article_detail = article.find(
            'p', class_='mdMN02Txt').text.replace("\n", "")
        article_detail_url = base_url + article.a['href']
        article_thumbnail_url = article.a.img['src']
        article_author = article.find(
            'p', class_='mdMN02Option').a.span.text.replace("\n", "")
        article_date = article.find('p', class_='mdMN02Option').time.text.replace(
            "\n", "").replace("-", ".")
        article_id = 'b' + \
            article_detail_url.replace(
                'https://news.kstyle.com/article.ksn?articleNo=', '')
